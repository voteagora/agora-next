...
- 2026-05-20 — Health scan complete — Full sweep of all 49 TanStack Start routes and all hooks/components for client-side imports of Vite-stubbed server files. Findings: (a) all delegate, staking, admin, forum, grants, retropgf, execution, notification-preference, and proposal routes are clean; (b) `src/app/proposals/draft/actions/*.ts` files have "use server" at file level and are accessed by client code only through `src/server/proposals/draft/` `createServerFn` wrappers — no issue; (c) `createProposalDraft.tsx` and RSC-style duna/forum page components remain dead code with no active importers; (d) no new issues found beyond the seatbelt degradation already documented. Scan confirmed the previously applied fixes are the full set of actionable runtime issues.
