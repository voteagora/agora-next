...
- [x] Phase F — Cutover (scripts swapped; all Next.js page/layout/loading/error/route files deleted; src/middleware.ts + src/instrumentation.{ts,node.ts} + src/pages/ + next.config.js deleted; next removed from package.json; ESLint updated to drop eslint-config-next; OG image routes ported from next/og → @vercel/og under src/routes/api/images/og/; assets moved to public/images/ for origin-URL fetching; build + typecheck both pass)
...
- 2026-05-19 — Cutover cleanup — Fixed dynamic header/profile links to avoid TanStack generated-route literal type failures, normalized object-shaped search params in the `next/navigation` shim, and prefixed OG route helper files with `-` so TanStack Router stops warning that they are route files. Verified `npm run build` passes.
- 2026-05-19 — Cutover typecheck — Added a local React `cache()` type augmentation for legacy RSC helpers rewritten by Vite, replaced Playwright's `@next/env` dependency with `dotenv`, normalized Vite asset imports from `.src` object access to URL strings, tightened bigint/null JSX guards, and fixed Safe publish callback typing. Verified `npm run typecheck` and `npm run build` pass.
- 2026-05-20 — Runtime bug fix — Moved `QueryClient` from module-level singleton to `useState(() => new QueryClient(...))` inside `Web3ProviderTanstack`. Root cause of second-route crash in regular Chrome (SSR shared state leaked stale query data → hydration mismatch → client navigation crash).
- 2026-05-20 — Runtime bug fix — Fixed `redirect()` in `RedirectOrConnect` component: `redirect()` from the next/navigation shim throws a TanStack router redirect (only valid in loader/beforeLoad); replaced with `useRouter().push()` for client-side navigation inside `useEffect`.
- 2026-05-20 — SSR warnings — Applied `useIsomorphicLayoutEffect` pattern to 4 files with direct `useLayoutEffect` calls that triggered SSR warnings: `useStableCallback.ts`, `useEffectEvent.tsx`, `PostTypeSelector.tsx`, `DunaContentRenderer.tsx`.
- 2026-05-20 — Server wrapper imports — Redirected all client-component imports that went directly to `"use server"` files (stubbed in client bundle) to their `src/server/**` wrappers: `OffchainCancel`, `CreateProposalFormClient`, `OffchainProposalAction`, all 5 draft stage components, both sponsor components. Created `src/server/proposals/offchainActions.ts` wrapper for `createOffchainProposal` + `cancelOffchainProposal`.
- 2026-05-20 — Lint fixes — Fixed `no-inner-declarations` in `financial-metrics.ts`; added `react/no-unknown-property` disable for `@vercel/og`'s `tw` prop in OG image routes; replaced `@ts-ignore` with `@ts-expect-error` in same routes.
- 2026-05-20 — Inline server action fix — Replaced inline `"use server"` functions in `CopelandProposalPage` and `OPProposalApprovalPage` with `createServerFn` wrappers from `@/server/proposals/votes`. Added `fetchSnapshotUserVotesForProposal` to `proposals/actions.tsx` and wrapped it in `votes.ts`. Inline server actions in component files (outside `src/app/`+`src/lib/`) are not auto-converted to RPCs by TanStack Start.
- 2026-05-20 — Server wrapper for utility hooks — Created `src/server/lib/abiUtils.ts` and `src/server/lib/transactionDecoder.ts` with `createServerFn` wrappers for `cachedGetContractAbi` and `cachedDecodeEnhanced`. Updated `useContractAbi`, `useExecutionTxLogs`, and `useTransactionDecoding` hooks to import from these wrappers instead of the stubbed `src/lib/` originals. Affected UI: admin scope details, execution event logs, proposal transaction decoding.
- 2026-05-20 — Known degradation (acceptable) — Seatbelt simulation (`checkExistingProposal`, `checkNewProposal`, `checkNewApprovalProposal`) is broken in client bundle (stubbed) but all call sites are wrapped in try/catch, so the feature degrades to an error toast/log rather than a crash. Fix requires bigint-safe createServerFn wrappers or a dedicated API route — deferred to post-cutover cleanup.
