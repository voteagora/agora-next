...
if (!fs.existsSync(diffDir)) {
  ...
  const routes = fs
    .readdirSync(diffDir)
    .filter((f) => fs.lstatSync(path.join(diffDir, f)).isDirectory());
    ...
    if (fs.existsSync(reportPath)) {
      ...
      if (reports.length > 0) {
        ...
        for (const item of renderableReports) {
          const icon =
            item.reason === "Data Drift" ? "🔤 Data Drift" : "📏 Layout Drift";
          const pText = item.productionText
            ? `\`${item.productionText.replace(/\|/g, "").substring(0, 30)}\``
            : "-";
          const bText = item.branchText
            ? `\`${item.branchText.replace(/\|/g, "").substring(0, 30)}\``
            : "-";
