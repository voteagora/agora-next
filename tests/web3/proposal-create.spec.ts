...
test.describe.serial("Proposal Creation Automation (Uniswap + Fawkes + Anvil)", () => {
  ...
  test("PROPOSAL-001: Automate creating a Uniswap proposal using Fawkes", async ({
    ...
    await page.getByLabel("Description").fill("This is an automated E2E proposal created via GitHub Actions and Anvil.");
    ...
    const addTransferBtn = page.getByRole("button", { name: /Transfer from the treasury/i });
    if (await addTransferBtn.isVisible()) {
        await addTransferBtn.click();
        
        // Fill out the TransferTransactionForm fields
        await page.getByLabel("Recipient").fill("0x1a9c8182c09f50c8318d769245bea52c32be35bc");
        await page.getByLabel(/Amount/i).fill("0");
        
        // Target the specific transaction description input
        await page.locator('input[name="transactions.0.description"]').fill("Automated Transfer");
    }

    // Submit the proposal on-chain
    const submitBtn = page.getByRole("button", { name: /Submit on-chain|Create proposal/i }).first();
    ...
    await expect(page.locator('text="Proposal created"').or(page.locator('text="Transaction created"'))).toBeVisible({ timeout: 30000 });
  });
});
