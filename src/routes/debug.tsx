  ...
  head: () => {
    ...
    return {
      meta: [
        ...
        {
          name: "description",
          content: `View configuration details for the current ${brandName} tenant instance.`,
        },
  ...
  return (
    <div className="flex items-start py-3 border-b border-line last:border-0">
      ...
      <div className="w-2/3">
        ...
        >
          <img alt="" width={14} height={14} src={linkIcon} />
          View on explorer
        </a>
      </div>
    </div>
...
function TenantDebugPage() {
  const tenant = Tenant.current();
  const { contracts, ui, namespace, slug, brandName, isProd } = tenant;

  const toggles = (
    ui as unknown as {
      _toggles?: Array<{ name: string; enabled: boolean; config?: unknown }>;
    }
  )._toggles;
      ...
      <Section title="Identifiers">
        <div className="bg-white rounded-lg border border-line overflow-hidden">
          <div className="p-6 space-y-0">
            <InfoRow
              label="Namespace"
              value={<span className="font-mono text-sm">{namespace}</span>}
            />
            <InfoRow
              label="Slug"
