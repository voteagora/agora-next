/*
 * TanStack Start port of src/app/api/v1/relay/vote/route.ts.
 * URL: POST /api/v1/relay/vote
 */
...
export const Route = createFileRoute("/api/v1/relay/vote")({
  server: {
    handlers: {
      POST: withApiAuth(async ({ request }) => {
        ...
        const { voteBySignatureApi } = await import(
          "@/app/api/v1/relay/vote/castVote"
        );
        ...
        try {
          ...
          return Response.json(voteTxHash);
        } catch (e: any) {
          appendServerTraceEvent({
            traceContext: withMiradorTraceStep(
              traceContext,
