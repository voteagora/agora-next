/*
 * TanStack Start port of src/app/api/v1/relay/delegate/route.ts.
 * URL: POST /api/v1/relay/delegate
 */
...
export const Route = createFileRoute("/api/v1/relay/delegate")({
  server: {
    handlers: {
      POST: withApiAuth(async ({ request }) => {
        ...
        const { delegateBySignatureApi } = await import(
          "@/app/api/v1/relay/delegate/delegate"
        );
        ...
        try {
          ...
          return Response.json(delegateTxHash);
        } catch (e: any) {
