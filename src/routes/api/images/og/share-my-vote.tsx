...
function OptionsResultsPanel({
  ...
  return (
    <div tw={"flex flex-col flex-shrink px-4"}>
      ...
      {totalOptions > 5 && (
        <div
          style={{ color: primary, backgroundColor: wash }}
...
function SingleOption({
  ...
  const percentage =
    totalVotingPower === BigInt(0)
      ? BigInt(0)
      : (votesAmountBN * BigInt(10000)) / totalVotingPower;
  ...
  return (
    <div style={{ gap: "4px" }} tw="flex flex-col last:mb-2">
      <div tw="flex justify-between font-semibold text-sm mb-1">
        ...
        <div style={{ gap: "4px", color: primary }} tw="flex items-center">
          <span style={{ gap: "4px" }} tw={"flex items-center"}>
            {formatNumber(votes, tenantToken.decimals, 2, true)}
          </span>
          <span style={{ color: tertiary }} tw={"ml-1"}>
            {percentage === BigInt(0)
              ? "0%"
      ...
      <ProgressBar
        barPercentage={Number(barPercentage)}
...
function ProgressBar({
  ...
  const progressBarWidth =
    Math.max(barPercentage / 100, barPercentage !== 0 ? 1 : 0).toFixed(2) + "%";
...
function generateVoteBars(
  ...
  return (
    <div tw="flex items-center justify-center w-full" style={{ gap: "10px" }}>
      {bars}
    </div>
  );
}

export const Route = createFileRoute("/api/images/og/share-my-vote")({
  server: {
    handlers: {
      GET: async ({ request }: { request: Request }) => {
        ...
        const [bgData, interBoldFont, interRegularFont, interSemiBoldFont] =
          await Promise.all([
            fetch(new URL(`${origin}/images/grid-share.png`)).then((r) =>
              r.arrayBuffer()
            ),
            fetch(new URL(`${origin}/fonts/Inter/Inter-Bold.ttf`)).then((r) =>
              r.arrayBuffer()
            ),
            fetch(new URL(`${origin}/fonts/Inter/Inter-Regular.ttf`)).then(
              (r) => r.arrayBuffer()
            ),
            fetch(new URL(`${origin}/fonts/Inter/Inter-SemiBold.ttf`)).then(
              (r) => r.arrayBuffer()
            ),
          ]);
                ...
                <div tw="h-full w-full flex flex-col p-8 relative rounded-lg">
                  <div tw="flex flex-col justify-between w-full h-full">
                    <div tw="flex flex-col">
                      <div tw="flex justify-between items-start w-full">
                        <div
                          ...
                          style={{
                            gap: "8px",
                            fontSize:
                              proposalType !== "APPROVAL" ? "54px" : "44px",
                          }}
                        >
                          <div tw="flex items-center" style={{ gap: "8px" }}>
                            <span style={{ color: primary }} tw="font-bold">
                              I voted
                            </span>
                            <span
                              style={{
                          ...
                          <span
                            style={{
                              color: primary,
                              fontSize:
                                proposalType !== "APPROVAL" ? "40px" : "32px",
                            }}
                            tw="font-normal"
                          >
                      ...
                      <div
                        style={{
                          borderColor: line,
                          marginTop:
                            proposalType !== "APPROVAL" ? "88px" : "40px",
                        }}
                      ...
                      >
                        {proposalType === "APPROVAL" ? (
                          ...
                          <div tw="flex flex-col p-9" style={{ gap: "8px" }}>
                            <div tw="flex justify-between w-full text-3xl font-semibold">
                              ...
                              <span style={{ color: negative }}>AGAINST</span>
                            </div>
                        ...
                        <div
                          style={{
                            ...
                            color: primary,
                            fontSize:
                              proposalType !== "APPROVAL" ? "24px" : "18px",
                          }}
                    ...
                    <div tw="flex justify-end items-center">
                      <div tw="flex" style={{ width: "155px", height: "40px" }}>
                        <svg
                          width="100%"
                          height="100%"
                          viewBox="0 0 62 16"
                        ...
                        >
                          <g clip-path="url(#clip0_4184_3154)">
                            ...
                            />
                            <path
                              d="M13.5214 -0.0410156H0.363224C0.162516 -0.0410156 0 0.121421 0 0.32203V1.15939C0 2.28751 0.918218 3.20528 2.04689 3.20528H13.5214C13.7229 3.20528 13.8854 3.04203 13.8854 2.84224V0.32203C13.8854 0.122233 13.7229 -0.0410156 13.5222 -0.0410156H13.5214Z"
                              fill="#171717"
                            />
                            <path
                            ...
                            />
                            <path
                              d="M47.5889 11.9143V2.90157H50.0876V5.09852C50.5759 3.44573 51.6095 2.67578 53.0559 2.67578C53.1282 2.67578 53.1875 2.73507 53.1875 2.80736V5.30482C52.8495 5.19192 52.4928 5.1172 52.0605 5.1172C51.1585 5.1172 50.1444 5.73690 50.1444 7.23944V11.9152H47.5889V11.9143Z"
                              fill="#171717"
                            />
                          </g>
                          <defs>
