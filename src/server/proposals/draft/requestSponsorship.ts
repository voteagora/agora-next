/**
 * TanStack Start createServerFn wrapper for requestSponsorship.
 *
 * Vite alias: "@/app/proposals/draft/actions/requestSponsorship" → this file.
...
const _serverRequestSponsorship = createServerFn({ method: "POST" })
  .inputValidator((data: Record<string, unknown>) => data)
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  .handler(async ({ data }): Promise<any> => {
    const { onSubmitAction: fn } = await import(
      "../../../app/proposals/draft/actions/requestSponsorship"
    );
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export const onSubmitAction = (data: any): Promise<FormState> =>
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  _serverRequestSponsorship({ data }) as any;
