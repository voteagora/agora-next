...
const getSortOptionValue = (sort: string) => {
  const sortOption = Object.values(myDraftsSortOptions).find(
    (option) => option.sort === sort
  );

  return sortOption?.value || myDraftsSortOptions.newest.value;
};

export default function MyDraftsSort() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const addSearchParam = useAddSearchParam();
  const sortParam = searchParams?.get("sort");
  const [selected, setSelected] = useState(
    sortParam || myDraftsSortOptions.newest.sort
  );
    ...
    <div className="relative w-full sm:w-fit">
      <Listbox value={selected} onChange={setSelected}>
        ...
        <Listbox.Options className="mt-3 absolute bg-wash border border-line p-2 rounded-2xl flex flex-col gap-1 z-10 w-max">
          {Object.values(myDraftsSortOptions).map((option) => (
