...
import {
  ARCHIVE_PROPOSAL_DEFAULTS,
  getArchiveDefaultForNamespace,
} from "@/app/proposals/data/archiveDefaults";
...
export function normalizeArchiveStandardProposal(
  ...
  const quorumValue = safeBigInt(
    proposal.quorum ??
      ...
      getArchiveDefaultForNamespace(ARCHIVE_PROPOSAL_DEFAULTS.quorum, namespace)
  );

  const votableSupplyValue = safeBigInt(
    proposal.votableSupply ??
      proposal.votable_supply ??
      getArchiveDefaultForNamespace(
        ARCHIVE_PROPOSAL_DEFAULTS.votableSupply,
        namespace
      )
  );

  const approvalThresholdValue = safeBigInt(
    proposal.approval_threshold ?? ARCHIVE_PROPOSAL_DEFAULTS.approvalThreshold
  );
