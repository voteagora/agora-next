    ...
    >
      ...
      <VStack className="px-4" >
        {proposalData.options
          ...
          .map((option) => {
            const value = parseFloat(formatUnits(option.values[0], 18)).toLocaleString("en-US");
            const valueETH = option.values[0] > 0 ? `{ value: ${formatEther(option.values[0])} ETH }` : undefined;

            return (
              <div key={option.description}>
                <p className="text-xs font-mono font-medium text-gray-af leading-4" >
                  <OptionDescription 
                    description={option.description} value={value} target={option.targets[0]} 
                  />
                </p>
                  <CodeChange
