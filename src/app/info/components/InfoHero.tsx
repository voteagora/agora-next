  ...
  return (
    <div className="relative">
      <Link
        target="_blank"
        href={link}
        className={`flex flex-col grow-0 p-1.5 border border-line rounded-[6px] shadow-[0px_3.044px_9.131px_0px_rgba(0,0,0,0.02),0px_1.522px_1.522px_0px_rgba(0,0,0,0.03)] hover:rotate-0 transition-all hover:z-10 hover:scale-110 ${!ui.customization?.customInfoSectionBackground ? "bg-neutral" : ""} ${className} ${isDisabled ? "opacity-50 pointer-events-none" : ""}`}
        style={
      ...
      >
        ...
        <div className="w-full flex flex-row justify-between gap-1 items-center text-xs font-medium text-secondary mt-1.5">
          ...
          <Image
            ...
            className={`self-start mt-1 ${ui.customization?.customInfoSectionBackground && ui.customization.customInfoSectionBackground !== "#FFFFFF" ? "brightness-0 invert" : ""} ${namespace === TENANT_NAMESPACES.SYNDICATE ? "brightness-0" : ""} ${isDisabled ? "opacity-50" : ""}`}
          />
        </div>
      </Link>
    </div>
