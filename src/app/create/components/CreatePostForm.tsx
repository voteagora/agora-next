  ...
  return (
    <FormProvider {...form}>
      ...
      <CardContent className="space-y-6 mt-4">
        <div>
          <Label className="text-xs font-semibold text-secondary" htmlFor="title">Title</Label>
          <Input
            id="title"
        ...
        {postType === "forum-post" && (
          <div>
            <Label className="text-xs font-semibold text-secondary" htmlFor="category">Category</Label>
            <select
              id="category"
              value={categoryId || ""}
              onChange={(e) =>
                setValue("categoryId", e.target.value ? Number(e.target.value) : undefined)
              }
        ...
        <div className="flex items-center justify-between pt-6 border-t">
          <div className="text-sm text-gray-500">
            {postType === "forum-post" && "This will post on the Syndicate forums"}
            {postType === "tempcheck" && (
              <div>
                {canCreateTempCheck ? (
