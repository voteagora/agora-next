...
- [ ] runtime `ProposalType` -> `LegacyProposalType`
      `LegacyProposalType` is already the canonical taxonomy name in `src/features/proposals/domain/`, and `src/lib/types.d.ts` now re-exports it explicitly. Central proposal model declarations, parsing/normalization helpers, vote utilities, vote API types, archive vote hooks, proposal repositories, DAO-node adapters, archive list helpers, dialog typing, and proposal UI registries now consume that name. Remaining old-name usage is concentrated in a few compatibility-facing surfaces plus the compatibility alias in `src/lib/types.d.ts`, comments, and ABI JSON metadata.
